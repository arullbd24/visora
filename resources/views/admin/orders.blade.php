        ...
        <table class="w-full table-auto text-sm">
            <thead>
                <tr class="bg-gray-200 text-gray-700">
                    ...
                    <th class="px-4 py-2">No.whatssapp</th>
            ...
            <tbody>
                @foreach ($orders as $order)
                    <tr class="border-b">
                        ...
                        <td class="px-4 py-2">{{ $order->whatsapp }}</td> <!-- ✅ FIXED -->
                            ...
                            <form action="{{ route('admin.orders.update', $order->id) }}" method="POST"
                                ...
                                <select name="status" class="text-sm px-2 py-1 rounded border">
                                    <option value="Menunggu Konfirmasi"
                                        {{ $order->status == 'Menunggu Konfirmasi' ? 'selected' : '' }}>Menunggu
                                    </option>
                                    <option value="Dikonfirmasi"
                                        {{ $order->status == 'Dikonfirmasi' ? 'selected' : '' }}>Konfirmasi</option>
                                    <option value="Ditolak" {{ $order->status == 'Ditolak' ? 'selected' : '' }}>Tolak
                                    </option>
                                </select>
                                <button type="submit"
