...
@section('content')
    <div class="max-w-7xl mx-auto pt-10 px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-10">
            <h1
                class="text-3xl sm:text-4xl font-extrabold text-center bg-gradient-to-r from-blue-700 via-blue-400 to-blue-700 bg-clip-text text-transparent mb-2 drop-shadow-lg">
                Hasil Rekomendasi Layanan</h1>
            <p class="text-center text-gray-500 text-sm sm:text-base">Temukan layanan terbaik sesuai kebutuhan Anda</p>
        </div>
        ...
        @else
            <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
                @foreach ($recommendations as $index => $rekom)
                    <div
                        class="bg-white p-6 rounded-2xl shadow-xl border border-gray-100 hover:shadow-2xl transition duration-300 relative overflow-hidden group flex flex-col justify-between">
                        <div>
                            <div
                                class="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-blue-500 via-purple-400 to-pink-400">
                            </div>
                            <h3 class="text-xl font-bold mb-2 text-blue-700 group-hover:text-blue-900 transition">
                                {{ $rekom['nama'] }}</h3>
                            <p class="text-sm text-gray-600 mb-3">{{ $rekom['deskripsi'] }}</p>
                            <p class="text-sm mb-2">
                                <strong>Skor Kecocokan:</strong>
                                <span
                                    class="{{ $rekom['score'] >= 70 ? 'text-green-600' : ($rekom['score'] >= 50 ? 'text-yellow-600' : 'text-red-600') }} font-bold">
                                    {{ $rekom['score'] }}%
                                </span>
                            </p>
                        ...
                        {{-- Carousel --}}
                        <div id="carousel-{{ $index }}" class="relative w-full mb-4 mt-2" data-carousel="slide">
                            <div
                                class="relative h-40 sm:h-48 overflow-hidden rounded-xl flex items-center justify-center bg-gray-50">
                                @php
                                    ...
                                    @if ($videoId)
                                        <div class="block duration-700 ease-in-out" data-carousel-item>
                                            <img src="https://img.youtube.com/vi/{{ $videoId }}/0.jpg"
                                                class="block w-full h-40 sm:h-48 object-cover rounded-xl shadow"
                            ...
                            <div class="absolute z-30 flex -translate-x-1/2 bottom-2 left-1/2 space-x-2">
                                @for ($i = 0; $i < 3; $i++)
                                    <button type="button"
                                        class="w-3 h-3 rounded-full bg-white border border-gray-300 shadow hover:bg-blue-400 transition"
                            ...
                            <button type="button"
                                ...
                                data-carousel-prev>
                                <span
                                    class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-white/40 group-hover:bg-blue-200 transition">
                        ...
                        <div class="flex flex-col sm:flex-row justify-center gap-2 mt-2">
                            <button onclick="openModal({{ $index }})"
                                class="bg-gray-200 text-sm text-gray-800 px-4 py-2 rounded-lg hover:bg-gray-300 transition font-semibold shadow w-full sm:w-auto">
                                Lihat Detail
                            </button>
                            <a href="{{ route('order.form', ['service_name' => $rekom['nama']]) }}"
                                class="bg-gradient-to-r from-blue-600 to-blue-400 text-white text-sm px-5 py-2 rounded-lg hover:from-blue-700 hover:to-blue-500 transition font-semibold shadow w-full sm:w-auto text-center">
                                Pesan Sekarang
                            </a>
                        </div>
                    </div>
            ...
            {{-- Modals (tidak perlu diubah, sudah responsif) --}}
            @foreach ($recommendations as $index => $rekom)
                <div id="modal-{{ $index }}"
                    ...
                    <div
                        class="bg-gradient-to-br from-blue-50 via-white to-purple-100 w-full max-w-xl rounded-3xl shadow-2xl p-6 sm:p-10 relative animate-fadeIn border-2 border-blue-200">
                        ...
                        <div class="flex items-center gap-4 mb-6">
                            <div
                                class="flex-shrink-0 w-14 h-14 rounded-full bg-gradient-to-tr from-blue-400 to-purple-400 flex items-center justify-center shadow-lg">
                                <svg class="w-8 h-8 text-white" fill="none" stroke="currentColor" stroke-width="2"
                                    viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" />
                                </svg>
                            </div>
                            <h2 class="text-xl sm:text-2xl font-bold text-blue-700">{{ $rekom['nama'] }}</h2>
                        </div>
                        <div
                            class="text-base text-gray-700 whitespace-pre-line leading-relaxed max-h-[60vh] overflow-y-auto px-1 pr-3 mb-4">
